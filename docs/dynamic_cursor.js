...
(function (global_object)
{
	...
	const PI = Math.PI;
	const TWO_PI = PI * 2;
	const PHYSICS_STEP = 1 / 240;   /* fixed sub-step, seconds           */
	const MAX_FRAME_TIME = 0.05;    /* ignore long pauses (tab switches) */
	const MUTATION_REFRESH_MS = 100;/* at most 10 style re-reads / sec   */
	const HIDE_CLASS = "dynamic_cursor_hidden";

	const SWING_PRESETS =
	{
		/* Your original feel: follows the movement, never overshoots. */
		smooth:
		{
			mode: "smooth",
			strength: 1.03,
			decay: 21.4,      /* how fast the push fades   (1/s) */
			follow: 6.3,      /* how fast the image follows (1/s) */
			max_angle: PI
		},
		/* Bouncy: overshoots and wobbles back into place. */
		spring:
		{
			mode: "spring",
			strength: 1.5,
		...
		/* Hanging object: reacts to speeding up / slowing down, can spin. */
		pendulum:
		{
			mode: "pendulum",
			strength: 0.13,
			gravity: 60,
			damping: 2.5,
			hang: false,      /* true = rests hanging straight down */
			max_angle: null
		}
	};
	...
	/* CSS swing value -> undefined (not set) | false | true | mode name */
	function parse_css_swing(text)
	{
	...
	/* Split "url(a) 1 2, url('b,c'), pointer" on top-level commas. */
	function split_cursor_list(text)
	{
		const parts = [];
		let depth = 0;
		let quote = "";
	...
	/*
	** value: false | true | "smooth" | "spring" | "pendulum" | { mode, ... }
	** base:  the already-normalized global swing (or null)
	** returns a full parameter object, or null for "no swing"
	*/
	function normalize_swing(value, base)
	{
		if (value === undefined)
			return (base);
		...
		{
			if (!SWING_PRESETS[value])
				throw (new Error("dynamic_cursor: unknown swing mode \"" +
					value + "\" (use smooth, spring or pendulum)"));
	...
	/* ------------------------------------------------------------------ */
	/* Physics (pure, no DOM, so it can be tested on its own)              */
	/* ------------------------------------------------------------------ */

	function create_swing_state()
	{
		return ({ angle: 0, velocity: 0, push: 0, speed_x: 0, speed_y: 0 });
	}

	function swing_rest_angle(params, lever_x, lever_y)
	{
	...
	/*
	** state:    from create_swing_state()
	** params:   normalized swing params
	** lever:    vector from hotspot to image center (px)
	** dx, dy:   how far the pointer moved since last step (px)
	** dt:       elapsed time (s)
	** returns true while still moving (so the loop can go to sleep)
	*/
	function step_swing(state, params, lever_x, lever_y, dx, dy, dt)
	{
		...
		/* Spread the movement over fixed sub-steps: same result at
		** 30, 60 or 240 frames per second. */
		...
		for (let i = 0; i < steps; ++i)
			sub_step(state, params, lever_x, lever_y, length_sq, rest,
				dx / steps, dy / steps, h);
	...
	function sub_step(state, params, lever_x, lever_y, length_sq, rest,
		dx, dy, h)
	{
		if (params.mode === "smooth")
		{
			/* The original model, written in continuous time. */
			state.push += params.strength *
				(lever_y * dx - lever_x * dy) / length_sq;
			state.push = clamp(state.push, params.max_angle);
			state.angle += (state.push - state.angle) *
				(1 - Math.exp(-params.follow * h));
			state.push *= Math.exp(-params.decay * h);
			return;
		}

		/* The lever turns with the image, so the push depends on the angle. */
		...
		{
			/* Pendulum: inertia reacts to changes of speed (acceleration). */
			...
			state.velocity += params.strength *
				(turned_y * (state.speed_x - old_x) -
					turned_x * (state.speed_y - old_y)) / length_sq;
		}

		const offset = wrap_angle(state.angle - rest);
		const pull = params.mode === "spring" ?
			params.stiffness * offset :
			params.gravity * Math.sin(offset);
		...
		if (params.max_angle != null && Math.abs(next) > params.max_angle)
		{
			...
			state.velocity *= -0.3;   /* soft bounce off the limit */
		}
		state.angle = rest + next;
	}

	/* ------------------------------------------------------------------ */
	/* Image cache (shared, so switching cursors never flickers)           */
	/* ------------------------------------------------------------------ */

	const image_cache = new Map();

	/* entry: { img, status: "loading" | "ready" | "error", width, height } */
	function get_image(src, on_settled)
	{
		...
		if (!entry)
		{
			...
			entry = { img: img, status: "loading", width: 0, height: 0,
				waiting: [] };
			...
			const settle = function (status)
			{
				...
				if (status === "error")
					console.warn("dynamic_cursor: could not load cursor " +
	...
	function normalize_cursor_definition(name, definition)
	{
		if (!definition || typeof definition !== "object" || !definition.src)
			throw (new Error("dynamic_cursor: cursor \"" + name +
				"\" needs at least a `src` (image url or <svg> string)"));
	...
	function deep_element_from_point(x, y)
	{
	...
	/* Scrollbars ignore CSS cursors, so we must show the native one there. */
	function is_over_scrollbar(element, x, y)
	{
	...
	function has_scrollbar(element)
	{
		return (element.clientWidth > 0 &&
			...
			(element.offsetWidth - element.clientWidth >
				element.clientLeft * 2 ||
				element.offsetHeight - element.clientHeight >
				element.clientTop * 2));
	}

	/* What the browser's `cursor: auto` would show: text or default. */
	function is_text_at(element, x, y, user_select)
	{
	...
	function start(options)
	{
		...
		let current = null;         /* resolved cursor being shown   */
		let mode = "native";        /* "image" | "native" | "none"   */
		...
		function mount()
		{
			...
			/* The shadow root keeps page CSS (img { max-width }, etc.) out,
			** and keeps our per-frame style changes invisible to observers. */
			style.textContent =
				":host {" +
		...
		/* Popovers live in the "top layer", above modal dialogs and
		** fullscreen elements. Re-showing puts us back on top of them. */
		function bring_to_top()
		{
			...
			{
				/* not connected yet, or not supported: z-index still works */
			}
		}
		...
		function set_mode(new_mode)
		{
			if (mode === new_mode)
				return;
			mode = new_mode;
			document.documentElement.classList.toggle(HIDE_CLASS,
				new_mode !== "native");
			if (image)
		...
		function read_styles(element)
		{
			/* Our "cursor: none" rule hides what the page asked for, so we
			** switch it off for one synchronous read. Nothing is painted in
			** between, so there is no flicker. */
			...
			const result =
			{
				...
				swing: computed.getPropertyValue("--cursor-swing") ||
					computed.getPropertyValue("--swing"),
		...
		function with_css_overrides(base, styles)
		{
		...
		/* returns { mode, definition? } */
		function resolve(element, styles)
		{
			...
			for (const part of split_cursor_list(styles.cursor))
			{
				...
				if (/^[a-z-]+$/i.test(part) === false)
					continue;   /* image-set() and friends: not supported */
				...
				const found = cursors.get(keyword) ||
				...
				if (!found || get_image(found.src).status === "error")
					return ({ mode: "native" });
				return ({ mode: "image",
					definition: with_css_overrides(found, styles) });
			}
			return ({ mode: "native" });
		}
		...
		function apply(result)
		{
			...
			if (entry.status === "loading")
				return;   /* keep showing the previous one until it's ready */
			...
			const swing = respect_reduced_motion && reduced_motion &&
				reduced_motion.matches ? null :
				normalize_swing(definition.swing, global_swing);
			const key = [definition.src, definition.x, definition.y,
				width, height, JSON.stringify(swing)].join("|");

			if (!current || current.key !== key)
			{
				if (!current || !current.swing || !swing ||
					current.swing.mode !== swing.mode)
				...
				image.style.transformOrigin =
					definition.x + "px " + definition.y + "px";
				...
				if (swing)
					swing_state.angle = swing_rest_angle(swing,
						current.lever_x, current.lever_y);
			}
			set_mode("image");
		}
		...
		function frame(now)
		{
			...
			const dt = last_time ?
				Math.min((now - last_time) / 1000, MAX_FRAME_TIME) : 1 / 60;
			const dx = pending_dx;
			const dy = pending_dy;
			...
			if (mode === "image" && current)
			{
				if (current.swing)
				{
					moving = step_swing(swing_state, current.swing,
						current.lever_x, current.lever_y, dx, dy, dt);
			...
			if (moving || needs_resolve || needs_hit_test)
				...
				last_time = 0;   /* sleep until the next event */
		}
		...
		function on_pointer_button()
		{
			/* :active / :focus rules may change the cursor */
			styles_dirty = true;
			needs_resolve = true;
			wake();
		}

		function on_leave_window(event)
		{
		...
		function on_mutation(records)
		{
			for (const record of records)
				if (record.attributeName === "open" && record.target !== host)
				{
					bring_to_top();   /* a dialog opened above us */
					break;
				}
			...
			const wait = Math.max(0,
				last_mutation_refresh + MUTATION_REFRESH_MS - Date.now());

			mutation_timer = window.setTimeout(function ()
			{
				mutation_timer = 0;
		...
		if (reduced_motion && reduced_motion.addEventListener)
			reduced_motion.addEventListener("change", refresh_everything,
				listen);
		...
		const instance =
		{
			/* Remove everything and give the native cursor back. */
			stop: function ()
			{
				if (!running)
					return;
			...
			/* Re-read CSS now (e.g. after changing classes in bulk). */
			refresh: refresh_everything,

			/* Add or replace a named cursor. */
			...
			/* Change the default swing for every cursor that doesn't set its own. */
	...
	const dynamic_cursor =
	{
		...
		/* exposed for testing / custom renderers */
